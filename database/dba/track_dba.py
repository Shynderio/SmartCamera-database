...
from database.dbo.track_dbo import TrackDBO as Track
...
class TrackDBA(MongoDBA):
    def __init__(self):
        super().__init__(db_config.SCHEMA["TRACKS"])

    # Private function
    def __find_one(self, condition: Dict[str, Any], session=None) -> Track:
        try:
            ...
            Logger("TrackDBA").log_error(f"Error when find one: {err}")
            return None
    ...
    def __find_many(
        ...
        try:
            ...
            return [Track.from_json_obj(data) for data in cursor]
        except ValueError as err:
            Logger("TrackDBA").log_error(f"Error when find many: {err}")
            return None

    def __find_by_id(self, id, session=None) -> Track:
        try:
            ...
            Logger("TrackDBA").log_error(f"Error when find by id: {err}")
            return None

    def __find_by_ids(self, ids: List[Any], session=None) -> List[Track]:
        try:
            ...
            Logger("TrackDBA").log_error(f"Error when finding by ids: {err}")
            return []

    def __insert_one(self, obj: Track, session=None) -> ObjectId:
        try:
            # Track.validate_multimedia(obj.multimedia)
            data = obj.model_dump(exclude_defaults=True)
            result = self.collection.insert_one(data, session=session)
            return result.inserted_id
        except ValueError as err:
            Logger("TrackDBA").log_error(f"Error when insert: {err}")
            return None

    def __insert_many(self, objs: List[Track], session=None) -> List[ObjectId]:
        try:
            # for obj in objs:
            #     Track.validate_multimedia(obj.multimedia)
            ...
            Logger("TrackDBA").log_error(f"Error when insert many: {err}")
            return None
    ...
    def __update_one(
        ...
        try:
            ...
            Logger("TrackDBA").log_error(f"Error when update one: {err}")
            return False
    ...
    def __update_many(
        ...
        try:
            ...
            print(err)
            Logger("TrackDBA").log_error(f"Error when update many: {err}")
            return False
    ...
    def __update_by_id(self, id, new_value: List[Any], session=None) -> bool:
        try:
            ...
            Logger("TrackDBA").log_error(f"Error when update by id: {err}")
            return False
    ...
    def __update_by_ids(
        ...
        try:
            ...
            Logger("TrackDBA").log_error(f"Error when update many by id: {err}")
            return False
    ...
    def __delete_one(self, condition: Dict[str, Any], session=None) -> bool:
        try:
            ...
            Logger("TrackDBA").log_error(f"Error when delete one: {err}")
            return False
    ...
    def __delete_many(self, condition: Dict[str, Any], session=None) -> bool:
        try:
            ...
            Logger("TrackDBA").log_error(f"Error when delete many: {err}")
            return False
    ...
    def __delete_by_id(self, id, session=None) -> bool:
        try:
            ...
            Logger("TrackDBA").log_error(f"Error when delete by id: {err}")
            return False
    ...
    def __delete_by_ids(self, ids: List[Any], session=None) -> bool:
        try:
            ...
            Logger("TrackDBA").log_error(f"Error when delete many by id: {err}")
            return False


    # Public function
    def find_one(self, condition: Dict[str, Any]) -> Track:
        result = self.transaction(self.__find_one, condition=condition)
        return result

    def find_many(self, condition: Dict[str, Any], n: int = None) -> List[Track]:
        result = self.transaction(self.__find_many, condition=condition, n=n)
        return result

    def find_by_id(self, id) -> Track:
        result = self.transaction(self.__find_by_id, id=id)
        return result

    def find_by_ids(self, ids: List[Any]) -> List[Track]:
    ...
    def update_tracks(self, tracks: List[Track]):
        ...
        new_values = [track.to_json() for track in tracks]
        result = self.transaction(self.__update_by_ids, ids= ids, new_values= new_values)
        return result
