...
from database.dbo.face_detection_dbo import FaceDetectionDBO as FaceDetection
...
class FaceDetectionDBA(MongoDBA):
    def __init__(self):
        super().__init__(db_config.SCHEMA["FACE_DETECTIONS"])

    # Private function
    def __find_one(self, condition: Dict[str, Any], session=None) -> FaceDetection:
        try:
            validated_condition = validate_condition(condition)
            result = self.collection.find_one(validated_condition, session=session)
            if result:
                return FaceDetection(**result)
            return None
        except ValueError as err:
            Logger("FaceDetectionDBA").log_error(f"Error when find one: {err}")
            return None
    ...
    def __find_many(
        ...
        try:
            ...
            return [FaceDetection.from_json_obj(data) for data in cursor]
        except ValueError as err:
            Logger("FaceDetectionDBA").log_error(f"Error when find many: {err}")
            return None

    def __find_by_id(self, id, session=None) -> FaceDetection:
        try:
            ...
            Logger("FaceDetectionDBA").log_error(f"Error when find by id: {err}")
            return None

    def __find_by_ids(self, ids: List[Any], session=None) -> List[FaceDetection]:
        try:
            ...
            face_detections = [FaceDetection(**result) for result in results]
            return face_detections
        except ValueError as err:
            Logger("FaceDetectionDBA").log_error(f"Error when finding by ids: {err}")
            return []

    def __insert_one(self, obj: FaceDetection, session=None) -> ObjectId:
        try:
            # FaceDetection.validate_multimedia(obj.multimedia)
            data = obj.model_dump(exclude_defaults=True)
            result = self.collection.insert_one(data, session=session)
            return result.inserted_id
        except ValueError as err:
            Logger("FaceDetectionDBA").log_error(f"Error when insert: {err}")
            return None

    def __insert_many(self, objs: List[FaceDetection], session=None) -> List[ObjectId]:
        try:
            # for obj in objs:
            #     FaceDetection.validate_multimedia(obj.multimedia)
            ...
            Logger("FaceDetectionDBA").log_error(f"Error when insert many: {err}")
            return None
    ...
    def __update_one(
        ...
        try:
            ...
            Logger("FaceDetectionDBA").log_error(f"Error when update one: {err}")
            return False
    ...
    def __update_many(
        ...
        try:
            ...
            print(err)
            Logger("FaceDetectionDBA").log_error(f"Error when update many: {err}")
            return False
    ...
    def __update_by_id(self, id, new_value: List[Any], session=None) -> bool:
        try:
            ...
            Logger("FaceDetectionDBA").log_error(f"Error when update by id: {err}")
            return False
    ...
    def __update_by_ids(
        ...
        try:
            ...
            Logger("FaceDetectionDBA").log_error(f"Error when update many by id: {err}")
            return False
    ...
    def __delete_one(self, condition: Dict[str, Any], session=None) -> bool:
        try:
            ...
            Logger("FaceDetectionDBA").log_error(f"Error when delete one: {err}")
            return False
    ...
    def __delete_many(self, condition: Dict[str, Any], session=None) -> bool:
        try:
            ...
            Logger("FaceDetectionDBA").log_error(f"Error when delete many: {err}")
            return False
    ...
    def __delete_by_id(self, id, session=None) -> bool:
        try:
            ...
            Logger("FaceDetectionDBA").log_error(f"Error when delete by id: {err}")
            return False
    ...
    def __delete_by_ids(self, ids: List[Any], session=None) -> bool:
        try:
            ...
            Logger("FaceDetectionDBA").log_error(f"Error when delete many by id: {err}")
            return False


    # Public function
    def find_one(self, condition: Dict[str, Any]) -> FaceDetection:
        result = self.transaction(self.__find_one, condition=condition)
        return result

    def find_many(self, condition: Dict[str, Any], n: int = None) -> List[FaceDetection]:
        result = self.transaction(self.__find_many, condition=condition, n=n)
        return result

    def find_by_id(self, id) -> FaceDetection:
        result = self.transaction(self.__find_by_id, id=id)
        return result

    def find_by_ids(self, ids: List[Any]) -> List[FaceDetection]:
    ...
    def update_face_detections(self, face_detections: List[FaceDetection]):
        ids = [face_detection.get_id() for face_detection in face_detections]
        new_values = [face_detection.to_json() for face_detection in face_detections]
        result = self.transaction(self.__update_by_ids, ids= ids, new_values= new_values)
        return result
